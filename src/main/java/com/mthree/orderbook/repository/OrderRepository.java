...
public interface OrderRepository extends JpaRepository<OB_Order, Integer> {
    ...
    @Query(value = "SELECT * FROM `ob_order` WHERE side = \"buy\" AND status = \"ACTIVE\" ORDER BY \"price\"", nativeQuery = true)
    List<OB_Order> findActiveBuyOrders();
    @Query(value = "SELECT * FROM `ob_order` WHERE side = \"sell\" AND status = \"ACTIVE\" ORDER BY \"price\" ASC", nativeQuery = true)
